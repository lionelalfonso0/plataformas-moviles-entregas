...
function calcularPromedio(listaDeNumeros) {
    const sum = listaDeNumeros.reduce((total, num) => total + num, 0);
    const prom = sum / listaDeNumeros.length;
    return prom;
}
...
function crearListaDeNumeros(inicio, fin) {
    ...
    return Array(longitud).fill().map((_, index) => inicio + index);
}
...
function encontrarPalabraMasCorta(listaDePalabras) {
    return listaDePalabras.reduce((palabraMasCorta, palabraActual) => 
        palabraActual.length < palabraMasCorta.length ? palabraActual : palabraMasCorta
    );
}
...
function enumerarLista(listaDePalabras) {
    ...
    const resultado = listaDePalabras.slice(0, -1).join(', ') + ' y ' + listaDePalabras.slice(-1);
